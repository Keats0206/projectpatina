...
const Navbar8 = ({ className, logoText, navLinks, primaryButtonLabel, isContained }: Navbar8Props) => {
  ...
  return (
    <section className={cn("", className)}>
      <div
        className={`${isContained ? "sticky top-0 z-10 bg-background/95 backdrop-blur" : "fixed top-0 z-500 bg-transparent"} w-full transition-colors duration-500`}
        ref={navRef}
      >
        <div className="container border-b">
          <div className="flex items-center justify-between gap-3.5 py-5">
            ...
            >
              ...
              />
              <span className="hidden md:inline-block">{logoText ?? LOGO.title}</span>
            </a>
            <NavigationMenu className="hidden lg:flex [&>div:nth-child(2)]:left-1/2 [&>div:nth-child(2)]:-translate-x-1/2">
              <NavigationMenuList>
                {(navLinks ? navLinks.map(l => ({ title: l.title, url: l.url })) : NAVIGATION).map((item, index) => (
                  <DesktopMenuItem
            ...
            <div className="flex items-center gap-3.5">
              {(primaryButtonLabel
                ? NAV_BUTTONS.map(b => b.variant === "default" ? { ...b, label: primaryButtonLabel } : b)
                : NAV_BUTTONS
              ).map((button, index) => (
                <Button
